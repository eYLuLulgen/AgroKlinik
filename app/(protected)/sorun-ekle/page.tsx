...
export default function SorunEklePage() {
  ...
  const handleAnalyze = async () => {
    if (file) {
      await analyzeImage(file, isPublic, plantName || 'Bilinmeyen Bitki');
    }
  };
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">🔬 AI Analiz Merkezi</h1>
          <p className="text-gray-600">Bitki fotoğrafınızı yükleyin, yapay zeka hastalığı tespit etsin</p>
        </div>
          ...
          <div className="space-y-6">
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all ${
                isDragActive ? 'border-green-500 bg-green-50' : preview ? 'border-green-300 bg-green-50/50' : 'border-gray-300 hover:border-green-400 hover:bg-green-50/30'
              }`}
            >
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                {preview ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <p className="text-lg font-medium text-gray-700">{isDragActive ? 'Bırakın!' : 'Fotoğraf yükleyin'}</p>
                      <p className="text-sm text-gray-500 mt-1">Sürükleyip bırakın veya tıklayarak seçin (max 10MB, JPEG/PNG/WebP)</p>
                    </div>
                  </div>
                )}
              </label>
            ...
            {/* Bitki adı */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Bitki Adı</label>
              <input
                type="text"
                value={plantName}
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{isPublic ? 'Herkes görebilir' : 'Sadece siz görebilirsiniz'}</p>
                </div>
                <button onClick={() => setIsPublic(!isPublic)} className={`relative w-14 h-8 rounded-full transition-colors ${isPublic ? 'bg-green-500' : 'bg-gray-300'}`}>
            ...
            >
              {isAnalyzing ? (
                <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Analiz Ediliyor...</>
              ) : (
                <>🔍 Analizi Başlat</>
              )}
            </button>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {currentAnalysis.severity && (
                        <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-medium border ${severityColors[currentAnalysis.severity] || severityColors['orta']}`}>
                          Şiddet: {currentAnalysis.severity}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {(currentAnalysis.prevention || []).length > 0 && (
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl border border-blue-200 p-6">
                    ...
                    <ul className="space-y-2">
                      {currentAnalysis.prevention.map((tip: string, index: number) => (
                    ...
                    <div className="space-y-3">
                      {(currentAnalysis.similarCases || []).map((caseItem) => (
                        <div key={caseItem.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${caseItem.status === 'çözüldü' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                            {caseItem.status}
                          </span>
                        </div>
                      ))}
